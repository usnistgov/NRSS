...
def _write_dev_plot(
    ...
    out = PLOT_DIR / f"{path_id}__{case.case_id}_bragg_2d_e{case.energy_eV:.1f}.png"
    fig.savefig(out, dpi=180, bbox_inches="tight")
    plt.close(fig)


def _evaluate_case(case: Bragg2DCase, nrss_path: ComputationPath) -> dict[str, object]:
    t0 = perf_counter()
    morph, metadata = build_bragg_2d_case_morphology(case, **_path_runtime_kwargs(nrss_path))
    metadata["vfrac"] = _to_numpy(morph.materials[1].Vfrac.copy())
...
def test_bragg_2d_lattice_pybind(case_id: str, nrss_path: ComputationPath):
