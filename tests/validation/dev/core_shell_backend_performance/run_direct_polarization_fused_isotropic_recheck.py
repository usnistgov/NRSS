...
SUMMARY_NAME = "direct_polarization_fused_isotropic_recheck_summary.json"
ROTATIONS = {
    "no_rotation": {
...
CASE_MATRIX = {
    "small_device__no_rotation": {
        "residency": "device",
        "startup_mode": "hot",
        "size_label": "small",
        "rotation_key": "no_rotation",
    },
    "small_device__rot_0_5_165": {
        "residency": "device",
        "startup_mode": "hot",
        "size_label": "small",
...
VARIANTS = {
    "baseline": {
        "label": "baseline",
        "description": "Current maintained direct_polarization float32 isotropic path.",
    },
    "item1_fused_float32_isotropic": {
        "label": "item1",
        "description": (
            "Drop isotropic_base_field and per-material isotropic_term; use "
            "float32 isotropic and anisotropic kernels that write directly into "
            "p_x/p_y/p_z."
        ),
    },
}
...
def _patch_item1_fused_float32_isotropic() -> None:
    runtime_cls = cupy_rsoxs_module.CupyRsoxsBackendRuntime
    original_run_single_energy_direct = runtime_cls._run_single_energy_direct_polarization
    original_compute_direct = runtime_cls._compute_direct_polarization
    ...
    def _direct_isotropic_kernel_float32(self, morphology, cp):
        return self._build_rawkernel_with_fallback(
            ...
            cache_key_base="direct_polarization_isotropic_complex64_float32_fused_candidate",
            source=r"""
            extern "C" __global__
            void direct_polarization_isotropic_complex64_float32_fused_candidate(
                const float* vfrac,
            ...
            """,
            kernel_name="direct_polarization_isotropic_complex64_float32_fused_candidate",
            requested_backend=self._rawkernel_backend_option(morphology, "direct_polarization_generic"),
        )

    def _direct_anisotropic_kernel_float32_fused(self, morphology, cp):
        return self._build_rawkernel_with_fallback(
            cp,
            family="direct_polarization_generic",
            cache_key_base="direct_polarization_anisotropic_complex64_float32_fused_candidate",
            source=r"""
            extern "C" __global__
            void direct_polarization_anisotropic_complex64_float32_fused_candidate(
                const float* vfrac,
                const float* s,
                const float* theta,
                const float* psi,
            ...
            """,
            kernel_name="direct_polarization_anisotropic_complex64_float32_fused_candidate",
            requested_backend=self._rawkernel_backend_option(morphology, "direct_polarization_generic"),
        )

    def patched_run_single_energy_direct_polarization(
        self,
        morphology,
        runtime_materials,
        energy,
        ...
        if (
            ...
            or cp.dtype(runtime_materials[0].Vfrac.dtype).name == "float16"
        ):
            return original_run_single_energy_direct(
                self,
                morphology,
                runtime_materials,
                energy,
    ...
    def patched_compute_direct_polarization(
        self,
        morphology,
        runtime_materials,
        energy,
        ...
        if (
            ...
            return original_compute_direct(
                self,
                morphology,
                runtime_materials,
                energy,
        ...
        mx = angle_plan.mx
        my = angle_plan.my
        isotropic_kernel = self._direct_isotropic_kernel_float32_candidate(morphology, cp)
        anisotropic_kernel = self._direct_anisotropic_kernel_float32_fused_candidate(
            morphology, cp
        )
        threads = 256
    ...
    runtime_cls._direct_isotropic_kernel_float32_candidate = _direct_isotropic_kernel_float32
    runtime_cls._direct_anisotropic_kernel_float32_fused_candidate = (
        _direct_anisotropic_kernel_float32_fused
    )
    runtime_cls._run_single_energy_direct_polarization = patched_run_single_energy_direct_polarization
    runtime_cls._compute_direct_polarization = patched_compute_direct_polarization


def _apply_variant_patch(variant: str) -> None:
    if variant == "baseline":
        return
    if variant == "item1_fused_float32_isotropic":
        _patch_item1_fused_float32_isotropic()
        return
...
def _variant_from_worker_case(case_path: Path, explicit_variant: str) -> str:
    ...
    if prefix == "baseline":
        return "baseline"
    if prefix == "item1":
        return "item1_fused_float32_isotropic"
...
def _parity_worker(case_path: Path, result_path: Path) -> int:
    ...
    try:
        ...
        _apply_variant_patch("item1_fused_float32_isotropic")
    ...
    *,
    ...
    return replace(
        ...
        worker_case=replace(
            ...
            label=variant_key,
            notes=(
                f"Direct-polarization fused-isotropic recheck variant={variant}. "
...
def _run_parity_check(*, run_dir: Path, gpu_index: int) -> dict[str, Any]:
    ...
    parity_case = replace(
        parity_case,
        label="parity__small_host__no_rotation",
        worker_warmup_runs=0,
        notes=(
            "Parity check for direct_polarization fused float32 isotropic "
            "candidate versus maintained baseline."
        ),
    )

    parity_dir = run_dir / "parity_case_results"
    parity_dir.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="dp_fused_iso_parity_", dir=parity_dir) as tmp_dir:
...
def _run_recheck(args: argparse.Namespace) -> int:
    if not has_visible_gpu():
        raise SystemExit(
            "No visible NVIDIA GPU found for the direct-polarization fused-isotropic recheck."
        )
    ...
    run_label = args.label or f"dp_fused_isotropic_{_timestamp()}"
    ...
    candidate_variant = "item1_fused_float32_isotropic"
    if baseline_variant in variants and candidate_variant in variants:
        for case_key in CASE_MATRIX:
            ...
            if case_key in speed_case_keys:
                ...
                decision["speed_ratio"] = float(candidate_primary / baseline_primary)
            ...
            summary["decision"][case_key] = decision
    ...
    for case_key, decision in summary["decision"].items():
        line = f"[decision] case={case_key}"
        if "speed_ratio" in decision:
            line += f" speed_ratio={decision['speed_ratio']:.4f}"
        if "memory_peak_ratio" in decision:
            line += f" memory_ratio={decision['memory_peak_ratio']:.4f}"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Development-only recheck for the direct_polarization fused float32 "
            "isotropic accumulation experiment."
        )
    )
    ...
    parser.add_argument(
        "--variants",
        default="baseline,item1_fused_float32_isotropic",
