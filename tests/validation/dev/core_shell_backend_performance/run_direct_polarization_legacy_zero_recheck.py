...
SUMMARY_NAME = "direct_polarization_legacy_zero_recheck_summary.json"
ROTATIONS = {
    "no_rotation": {
...
VARIANTS = {
    "baseline": {
        "label": "baseline",
        "description": "Current maintained legacy_zero_array runtime staging.",
    },
    "item1_runtime_zero_contract": {
        "label": "item1",
        "description": (
            "Detect legacy zero S/theta/psi fields at runtime, stage Vfrac only, "
            "and route those materials through the isotropic direct-path handling."
        ),
    },
    "item2_runtime_zero_contract_bucketed": {
        "label": "item2",
        "description": (
            "Apply the runtime zero-field contract and then split isotropic and "
            "anisotropic materials into separate direct-path loops."
        ),
    },
}
...
def _patch_runtime_zero_contract_bucketed() -> None:
    runtime_cls = cupy_rsoxs_module.CupyRsoxsBackendRuntime
    ...
    def patched_compute_direct_polarization(
        ...
        mx = angle_plan.mx
        my = angle_plan.my
        anisotropic_kernel = self._direct_polarization_kernel(
            morphology, cp, runtime_materials[0].Vfrac.dtype
        )
        isotropic_kernel = self._direct_isotropic_kernel_float32(morphology, cp)

        isotropic_materials = tuple(material for material in runtime_materials if material.is_full_isotropic)
        anisotropic_materials = tuple(
            material for material in runtime_materials if not material.is_full_isotropic
        )

        for material in isotropic_materials:
            isotropic_diag, _, _ = self._material_optical_scalars(material, energy)
            isotropic_kernel(
                ((material.Vfrac.size + 255) // 256,),
                (256,),
                (
                    material.Vfrac,
                    isotropic_diag,
                    ...
                    p_x,
                    p_y,
                    np.uint64(material.Vfrac.size),
                ),
            )

        for material in anisotropic_materials:
...
def _apply_variant_patch(variant: str) -> None:
    ...
    if variant == "item1_runtime_zero_contract":
        ...
        return
    if variant == "item2_runtime_zero_contract_bucketed":
        _patch_runtime_zero_contract()
        _patch_runtime_zero_contract_bucketed()
        return
...
def _variant_from_worker_case(case_path: Path, explicit_variant: str) -> str:
    ...
    if prefix == "item2":
        return "item2_runtime_zero_contract_bucketed"
    ...
    *,
    ...
    return replace(
        ...
        worker_case=replace(
            ...
            label=variant_key,
            notes=(
                f"Direct-polarization legacy-zero recheck variant={variant}. "
...
def _run_parity_check(*, run_dir: Path, gpu_index: int) -> dict[str, Any]:
    ...
    parity_case = replace(
        ...
        notes=(
            "Parity check for the runtime legacy-zero contract versus the "
            "maintained legacy_zero_array baseline."
        ),
    )

    parity_dir = run_dir / "parity_case_results"
    parity_dir.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="dp_legacy_zero_parity_", dir=parity_dir) as tmp_dir:
...
def _run_recheck(args: argparse.Namespace) -> int:
    if not has_visible_gpu():
        raise SystemExit("No visible NVIDIA GPU found for the direct-path legacy-zero recheck.")
    ...
    run_label = args.label or f"dp_legacy_zero_{_timestamp()}"
                ...
                if case_key in speed_case_keys:
                    ...
                    for segment in ("A2", "B"):
                        if (
                            ...
                            and segment in summary["speed_cases"][variant][case_key].get(
                                "segment_seconds_stats", {}
                            )
                        ):
                            ...
                            candidate_seg = summary["speed_cases"][variant][case_key][
                                "segment_seconds_stats"
                            ][segment]["median"]
                            decision[f"{segment.lower()}_ratio"] = float(candidate_seg / baseline_seg)
    ...
    for variant, variant_summary in summary["decision"].items():
        for case_key, decision in variant_summary.items():
            ...
            if "a2_ratio" in decision:
                line += f" a2_ratio={decision['a2_ratio']:.4f}"
            if "b_ratio" in decision:
                line += f" b_ratio={decision['b_ratio']:.4f}"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Development-only recheck for the direct_polarization legacy-zero "
            "runtime staging experiments."
        )
    )
    ...
    parser.add_argument(
        "--variants",
        default="baseline,item1_runtime_zero_contract,item2_runtime_zero_contract_bucketed",
