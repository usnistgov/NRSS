...
PLOT_DIR = REPO_ROOT / "test-reports" / "sphere-centering-dev"
...
def _sphere_center(mode: str) -> tuple[float, float, float]:
    nz, ny, nx = SHAPE
    if mode == "half_voxel":
        return (nz - 1) / 2.0, (ny - 1) / 2.0, (nx - 1) / 2.0
    if mode == "voxel_centered":
        return float(nz // 2), float(ny // 2), float(nx // 2)
    raise AssertionError(f"Unknown centering mode: {mode}")


def _sphere_and_vacuum_vfrac(diameter_nm: float, centering_mode: str) -> tuple[np.ndarray, np.ndarray]:
    nz, ny, nx = SHAPE
    radius_vox = float(diameter_nm) / (2.0 * PHYS_SIZE_NM)
    cz, cy, cx = _sphere_center(centering_mode)
...
def _build_morphology(centering_mode: str) -> Morphology:
    sphere_vfrac, vacuum_vfrac = _sphere_and_vacuum_vfrac(DIAMETER_NM, centering_mode)
    zeros = _zero_field()
...
def _write_plot(results: dict[str, dict]) -> Path:
    ...
    for label, color in CENTERING_MODES:
        res = results[label]
        q = res["q"]
        sim = res["sim_norm"]
        ana = res["ana_norm"]
    ...
    reference_label = CENTERING_MODES[0][0]
    ...
    for label, _ in CENTERING_MODES:
        metrics = results[label]["metrics"]
        minima = results[label]["minima"]
        center = results[label]["center"]
        build_seconds = results[label]["build_seconds"]
        run_seconds = results[label]["run_seconds"]
        note_lines.append(
            (
                f"{label}: center={center} p95={metrics['p95_log_abs']:.4f} "
                f"max={metrics['max_log_abs']:.4f} minima={minima[:4]} "
    ...
    fig.suptitle("Sphere Centering Diagnostic")
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    out = PLOT_DIR / "sphere_centering_d127_e285.png"
...
@pytest.mark.gpu
@pytest.mark.slow
def test_sphere_centering_diagnostic():
    if not _has_visible_gpu():
        pytest.skip("No visible NVIDIA GPU found for sphere centering diagnostic.")
