...
`UPGRADE_ROADMAP.md` and the historical backend-upgrade guide.
...
- `README.md`
  - routing index for optimization docs, benchmark guidance, accepted state,
    minimal histories, and remaining open ideas
- `direct_polarization_minimal_history.md`
  - compact path-specific history for
...
3. Detailed performance work and timing methodology are intentionally separated
   into the optimization documentation tree rooted at `README.md`.
...
For the current approximation routing, support posture, and half-input caveats,
see `fast_approximations.md`. Historical design detail is preserved in
...
For the archived full resumption blueprint, including first-pass
implementation scope, internal helper refactor recommendations, and the
analytical-sphere full-`3D`-versus-collapsed-`3D` validation proposal, see
`archive/CUPY_RSOXS_Z_COLLAPSE_PROPOSAL.md`.
...
Detailed timing methodology, benchmark caveats, and experiment history now live
in the optimization tree under `README.md`.
