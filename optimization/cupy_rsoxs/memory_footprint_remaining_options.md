...
Readout
  ...
  - The current runtime zero-field shortcut is host-only. Device-resident legacy_zero_array materials do not get compacted, so device runs keep zero S/theta/psi arrays resident. See dev/NRSS/src/NRSS/backends/cupy_rsoxs.py:681 and dev/NRSS/src/NRSS/backends/cupy_rsoxs.py:744.
...
# Possible Experiments
  ...
  2. Generalize the runtime zero-field shortcut to device residency, or normalize zero arrays to explicit isotropic before device runs. On the current legacy_zero_array medium device cases this should save about 1536 MiB by dropping zero S/theta/psi fields for the isotropic core and matrix. The same idea already paid off on host-side accepted rechecks. See
  dev/NRSS/src/NRSS/backends/cupy_rsoxs.py:681, dev/NRSS/test-reports/core-shell-backend-performance-dev/tc_mem10_legacy_zero_20260407/tensor_coeff_legacy_zero_recheck_summary.json:10139, and dev/NRSS/optimization/cupy_rsoxs/archive/CUPY_RSOXS_DIRECT_POLARIZATION_OPTIMIZATION.md:2537.
  3. Add a tensor_coeff_low_memory projection mode that avoids full-volume basis_x/basis_y. The right experiment is a fused detector-projection kernel that reads fft_nt[0..4] directly and writes the 2D projection, instead of first materializing 3D basis triplets. This is the main rotation-fix experiment; it should remove the extra 1536 MiB general-angle step
  and likely lower the no-rotation peak too. See dev/NRSS/src/NRSS/backends/cupy_rsoxs.py:1875.
  4. Add a cyrsoxs-style memory_mode='low' for host residency. Stage and accumulate per energy, then release staged morphology arrays before FFT/projection. If needed, go one material at a time. This targets the current host staging footprint, about 1792 MiB on medium, at a predictable transfer/speed cost. See dev/NRSS/src/NRSS/backends/cupy_rsoxs.py:744 and
  dev/cyrsoxs/src/cudaMain.cu:1231.
  5. Diagnose the shared device-only +512 MiB energy step with FFT-plan instrumentation. The explicit device result buffer is too small to explain that jump in the 1 -> 3 energy report, so this looks more like cuFFT plan/workspace or pool high-water than live result data. Run A/B with explicit plan creation, plan-cache clear between energies, and pool
  metrics after energy 1 vs 2.
  6. Do not spend more time first on allocator tricks or Segment C-only in-place work. The tensor in-place Segment C recheck already failed to lower peak memory, and pool-off only cut about 56 MiB while slowing the direct path by about 2.2x to 2.4x. See dev/NRSS/tests/validation/dev/core_shell_backend_performance/README.md:116 and dev/NRSS/optimization/
  cupy_rsoxs/archive/CUPY_RSOXS_DIRECT_POLARIZATION_OPTIMIZATION.md:2249.

  If I were doing this in order, I would implement host result streaming first, then device zero-field compaction, then a fused low-memory tensor_coeff detector projection mode. Those three directly target the two measured pathologies without depending on allocator behavior.