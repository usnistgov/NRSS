...
- `optimization/cupy_rsoxs/README.md`
  - optimization routing index for accepted state, benchmark authority, compact
    histories, and remaining open ideas

Legacy mixed-role documents and their historical roles are now superseded by
the files above:
...
3. `cupy-rsoxs` compute/runtime work now exists in-repo; stable backend
   behavior is documented in `optimization/cupy_rsoxs/backend_spec.md`.
...
6. Speed optimization is now documented separately in
   `optimization/cupy_rsoxs/README.md` so timing history no longer dominates
   the repo-wide plan.
...
1. The canonical detailed implementation and validation proposal for this work
   now lives in `optimization/cupy_rsoxs/fast_approximations.md`, with the
   archived long-form proposal preserved under
   `optimization/cupy_rsoxs/archive/CUPY_RSOXS_Z_COLLAPSE_PROPOSAL.md`.
...
The original backend upgrade guide was explicitly prep-only. It set the
...
1. historical backend-upgrade guide material
2. `pyproject.toml`
3. `environment.yml`
...
13. These command fragments are historical verification snapshots only.
    ...
    surface, which is documented in
    `optimization/cupy_rsoxs/backend_spec.md`.
...
1. `cupy-rsoxs` compute/runtime implementation now exists; timing cleanup is in
   ...
   instrumentation follow-up. See `optimization/cupy_rsoxs/README.md` for the
   current detailed routing and compact state.
...
2. The approved mixed-precision implementation plan now lives in:
   - `optimization/cupy_rsoxs/backend_spec.md` for the stable option surface
     and execution
     contract,
   - `optimization/cupy_rsoxs/README.md` plus
     `optimization/cupy_rsoxs/benchmarking_guide.md` for the implementation
     order, timing interpretation, and optimization routing.
   - current lock-ins:
